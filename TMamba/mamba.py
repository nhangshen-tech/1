...
from Transfor import CrossAttention,SelfAttention
...
class Encoder(nn.Module):
    def __init__(self, encoder_layer, num_layers, norm=None, args=None):
        ...
        # self.sa_layers = [getattr(args, 'self_attn_layers', num_layers) - 1]
        self.sa_layers = [0, 2]
    ...
    def forward(self, src, mask: Optional[Tensor] = None,
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if idx in self.sa_layers:
                sa_out = self.self_attention(output)
                gate = self.gate_sa(output)
                output = output + gate * sa_out
...
class Decoder(nn.Module):
    def __init__(self, decoder_layer, num_layers, norm=None, return_intermediate=False, args=None):
        ...
        self.gate_ca = nn.Sequential(
            nn.Linear(args.hidden_dim, args.hidden_dim),
            nn.Sigmoid()
        )
        # self.ca_layers = getattr(args, 'ca_layers', [num_layers - 1])
        self.ca_layers = [num_layers - 2]
    ...
    def forward(self, tgt, memory,
        ...
        for idx, layer in enumerate(self.layers):
            output = layer(
                self.with_pos_embed(output, pos),
                self.with_pos_embed(memory, query_pos)
            ) + output

            if idx in self.ca_layers:
                ca_out = self.cross_attention(output, memory, memory, key_padding_mask=memory_key_padding_mask)
                gate = self.gate_ca(output)
                output = output + gate * ca_out
        ...
        if self.return_intermediate:
            return torch.stack(intermediate)  # [num_layers, L, B, C]
        return output.unsqueeze(0)  # [1, L, B, C]
